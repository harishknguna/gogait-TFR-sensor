    ...
    for veri, version in enumerate(version_list):
        ...
        for ntype in numTypeTopo: 
            ...
            if ntype == 'real': 
                ...
                pwr_data = np.abs(np.exp(tfr_all))
            elif ntype == 'expPhase': # sqrt(real2 + imag2)
                pwr_data = np.angle(np.exp(tfr_all))
            ...
            for f, fn in enumerate(fband): 
                ...
                fig.tight_layout()
                report.add_figure(fig, title = fn + '_' + ntype, replace = True)
...
#%           
    ...
    report.title = 'Group sub TF topomaps : ' + evnt +'_' + version + '_' + waveType +'_'+ numType +'_'+ baseline+ '_' + corr + '_' + avgType
    ...
    report.save(report_fname+'_' + evnt +'_' + version + '_' + waveType +'_'+ numType +'_'+ baseline+ '_' + corr + '_' + avgType +'_all.html', overwrite=True)            
