...
## note: always take baseline uncorr version, for Cue
...
for ei, evnt in enumerate(event_type):
    ...
    for veri, version in enumerate(version_list):
        ...
        for ci, condi in enumerate(condi_name): 
            ...
            for sub_num, subject in enumerate(config_for_gogait.subjects_list): 
                ...
                #     extension =  condi_name[ci] +'_' + event_type[ei] +'_' + version + waveType +'_ITC_bslnCorr-tfr.h5'
                # elif baseline == 'bslnNoCorr':
                #     print('Importing the TFR_itc from disk')
                #     extension =  condi_name[ci] +'_' + event_type[ei] +'_' + version + waveType +'_ITC-tfr.h5'
        ...
        pwr_array_GOc = tf_pwr_array_avg_sub_all_condi[0,:,:,:]
        ...
        pwr_array_GOu = tf_pwr_array_avg_sub_all_condi[1,:,:,:]
        ...
        pwr_array_NoGo = tf_pwr_array_avg_sub_all_condi[2,:,:,:]
        ...
        for f, fn in enumerate(fband): 
            ...
            tmin = [-0.5, -0.3, 0.0, 0.20, 0.40] # in s
            tmax = [-0.3, 0.0, 0.2, 0.40, 0.60] # in s
            nT = len(tmin)
            
            tmin_ms = [- 500, -300, 0, 200, 400] # in ms
                ...
                for cols in range(nT): 
                    ...
                    if rows == 0:
                        ...
                        im = power_GOc.plot_topomap(baseline = None,
                                                    ...
                                                    vlim=(vmin, vmax), cmap = 'RdBu_r', colorbar= True)
                        if cols == 0:
    ...
    report.title = 'Group sub TF topomaps : ' + evnt + '_' + baseline + '_' + version + '_' + waveType 
